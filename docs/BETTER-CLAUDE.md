# Guide for Claude: Implementing the SaaS Starter Kit

**Purpose:** This document provides comprehensive context for Claude Code to implement this project across multiple sessions.
...
1. **Granular Commits** - Every change should be small, focused, and easily revertible
...
```bash
# If adding new pattern or changing architecture
git add docs/ARCHITECTURE.md
git commit -m "docs(architecture): document password hashing approach"
```

**6. Continue with next small piece:**
```bash
...
- [ ] Set up Turborepo
- [ ] Configure TypeScript (base, nextjs, node)
...
- [ ] Drizzle config
- [ ] Initial schema (users, sessions, accounts, verifications)
...
### App: API (Auth)
- [ ] User repository (Drizzle + In-memory)
...
- [ ] ADR for any major decisions made during implementation
...
3. **Validate everything** - Never trust external data
4. **Document as you go** - Docs in same commit as code
5. **Ask when unclear** - Better to ask than assume

**End of Better Claude Guide**
